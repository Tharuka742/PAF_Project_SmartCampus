  ...
  return (
    <article className="glass-card group relative flex h-full flex-col overflow-hidden rounded-2xl border border-slate-700/70 bg-slate-900/95 shadow-[0_10px_28px_rgba(2,6,23,0.28)] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_18px_36px_rgba(2,6,23,0.34)]">
      ...
      <div className="relative h-44 w-full overflow-hidden border-b border-slate-700/70 bg-slate-800">
        {resource.imageUrl ? (
          ...
          <div className="flex h-full w-full items-center justify-center bg-gradient-to-br from-slate-800 to-slate-900">
            <span className="text-5xl font-black uppercase tracking-tighter text-slate-500/70">
              {resource.type?.substring(0, 3)}
            </span>
          </div>
        )}
        ...
        <div
          className={`absolute right-3 top-3 rounded-full border px-3 py-1 text-xs font-semibold uppercase tracking-wide ${getStatusColor(
            resource.status
          )}`}
        >
      ...
      <div className="flex flex-grow flex-col p-6">
        <div className="mb-4">
          <h3 className="line-clamp-2 text-xl font-bold leading-tight text-slate-100 group-hover:text-cyan-300">
            {resource.name}
          </h3>
        ...
        <div className="flex flex-grow flex-col space-y-3 text-sm">
          ...
          <div className="flex items-center gap-3 rounded-xl border border-slate-700/70 bg-slate-800/55 px-3 py-2.5 text-slate-200">
            ...
            <span>
              ...
              <strong className="text-slate-100">{resource.capacity}</strong> people
            </span>
          </div>
          ...
          {(resource.amenities?.length > 0 || resource.description) && (
            <div className="mt-1 space-y-2 border-t border-slate-700/70 pt-3">

              {resource.amenities?.length > 0 && (
                <div className="flex items-start gap-2 text-slate-300">
                  <Box size={14} className="mt-0.5 text-violet-300" />
                  <span className="text-xs">
                    <strong className="text-slate-200">Amenities:</strong>{" "}
        ...
        {canManage && (
          <div className="mt-5 flex gap-3 border-t border-slate-700/70 pt-4">
            <button
              ...
              className="flex-1 flex items-center justify-center gap-2 rounded-xl border border-indigo-400/35 bg-indigo-500/18 px-4 py-2.5 text-sm text-indigo-200 hover:bg-indigo-500/30"
            >
              <Edit size={16} /> Edit
            </button>
            ...
            <button
              ...
              className="w-12 flex items-center justify-center rounded-xl border border-rose-400/40 bg-rose-500/12 text-rose-200 hover:bg-rose-500/25"
            >
              <Trash2 size={16} />
            </button>
          </div>
